      ...
      @php
          ...
          foreach ($books as $book) {
            if (!empty($_GET['serch'])) {
                # code...
                // if(
                //     stripos($book->titre,$_GET['search']) || 
                //     stripos($book->auteur,$_GET['search'])
                //     )
                // {
                //     $search['titre'] = $book->titre;
    
                // }
            }
            // var_dump(in_array($_GET['search']));
            // var_dump( $book->titre )
        //   var_dump([$book->titre][0]);
        //   var_dump(in_array($_GET['search'],[$book->titre]));
        $search= $_GET['search'] ;

          $var = $book->where('titre','like','%$search%');
          var_dump($var);
          
            }
      @endphp
        <form class="d-flex" action="" method="GET" role="search">
            <input class="form-control me-2" value="{{$_GET['search']}}" name="search" placeholder="Search" aria-label="Search">
